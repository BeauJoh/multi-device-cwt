...
static std::string getenv_or(const char* name, const char* def) {
  ...
  if (!v || std::string(v).empty()) return std::string(def);
  return std::string(v);
}
...
static void append_csv(
  ...
  std::string impl = getenv_or("CWT_IMPL", "HIP / HIPCC");
  std::string machine = getenv_or("CWT_MACHINE", "UnknownMachine");
  std::string backend = getenv_or("CWT_BACKEND", "HIP");
...
int main(int argc, char** argv) {
  ...
  int visible = 0;
  HIP_CHECK(hipGetDeviceCount(&visible));
  if (visible <= 0) throw std::runtime_error("No HIP devices found");
  ...
  auto h_fx = build_signal(N, B);
  auto h_scales = linspace(0.01, 0.10, A);
  auto h_trans = linspace(-1.0, 1.0, N);
  ...
  std::vector<hipStream_t> streams(physical_workers);

  for (int w = 0; w < physical_workers; ++w) {
    HIP_CHECK(hipSetDevice(w));
    HIP_CHECK(hipStreamCreate(&streams[w]));
    HIP_CHECK(hipMalloc(&d_fx[w], sizeof(real_t) * B * N));
    HIP_CHECK(hipMalloc(&d_scales[w], sizeof(real_t) * A));
    HIP_CHECK(hipMalloc(&d_trans[w], sizeof(real_t) * N));

    HIP_CHECK(hipMemcpyAsync(d_fx[w], h_fx.data(), sizeof(real_t) * B * N,
                             hipMemcpyHostToDevice, streams[w]));
    HIP_CHECK(hipMemcpyAsync(d_scales[w], h_scales.data(), sizeof(real_t) * A,
                             hipMemcpyHostToDevice, streams[w]));
    HIP_CHECK(hipMemcpyAsync(d_trans[w], h_trans.data(), sizeof(real_t) * N,
                             hipMemcpyHostToDevice, streams[w]));
  }
  ...
  // hipMalloc is a synchronous, blocking call; doing this inside the timed
  // loop (interleaved with hipSetDevice() switches across devices) would
  // count allocator overhead as if it were compute/transfer time, and that
  // overhead is paid once per task -- i.e. it scales up with --gpus even
  // though each task's chunk of work is shrinking, which can make more
  // GPUs look slower for reasons that have nothing to do with the actual
  // kernel. Matches the pattern already used for d_fx/d_scales/d_trans.
  for (int i = 0; i < int(tasks.size()); ++i) {
    int w = args.mode == "single" ? 0 : (i % physical_workers);
    HIP_CHECK(hipSetDevice(w));
    int a_count = tasks[i].a1 - tasks[i].a0;
    HIP_CHECK(hipMalloc(&d_out[i], sizeof(real_t) * B * a_count * N));
  }
  ...
  // avoids that.
  {
    ...
    while (remaining > 0) {
      for (int w = 0; w < physical_workers; ++w) {
        ...
        HIP_CHECK(hipSetDevice(w));
        hipError_t st = hipStreamQuery(streams[w]);
        if (st == hipSuccess) {
          dev_done[w] = true;
  ...
  {
    std::string impl_for_timeline = getenv_or("CWT_IMPL", "HIP / HIPCC");
    std::string machine_for_timeline = getenv_or("CWT_MACHINE", "UnknownMachine");
    for (int w = 0; w < physical_workers; ++w) {
  ...
  for (int w = 0; w < physical_workers; ++w) {
    HIP_CHECK(hipSetDevice(w));
    HIP_CHECK(hipFree(d_fx[w]));
    HIP_CHECK(hipFree(d_scales[w]));
    HIP_CHECK(hipFree(d_trans[w]));
